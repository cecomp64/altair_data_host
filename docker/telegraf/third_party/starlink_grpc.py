# Vendored unmodified from https://github.com/sparky8512/starlink-grpc-tools
# (main branch, retrieved 2026-08-16), public domain (Unlicense) - see that
# repo's LICENSE. Talks to the Starlink dish's local gRPC API
# (192.168.100.1:9200, the same one the mobile app uses) via gRPC server
# reflection (through the `yagrc` package), so no pregenerated SpaceX
# protobuf stubs are needed. Committed as a file rather than fetched at
# Docker build time so an image build never depends on GitHub being
# reachable/unchanged - see docker/telegraf/Dockerfile.
#
# This module includes dish *control* functions (reboot(), set_stow_state(),
# set_sleep_config(), reset_obstruction_map(), etc.) that scripts/read_starlink.py
# never calls - only the read-only status_data()/history_stats() are used.
# Vendored whole rather than trimmed down, same as vendor/waveshare-epaper/
# at the repo root, since hand-extracting a subset risks quietly breaking
# something the reflection-based parsing depends on.
